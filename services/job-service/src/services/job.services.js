...
export const updateJob = async ({ jobId, userId, requestId, updates }) => {
  try {
    ...
    const job = await Job.findOneAndUpdate(
      ...
      {
        $set: { ...value, updatedBy, "dates.lastUpdated": new Date() },
        $inc: { version: 1 },
      },
...
export const listJobs = async ({ filters, requestId }) => {
  ...
  if (queryFilters.location) {
    ...
    if (queryFilters.location.country)
      query["location.country"] = new RegExp(
        queryFilters.location.country,
        "i"
      );
    if (queryFilters.location.isRemote)
  ...
  if (queryFilters.skills)
    query["skills.name"] = {
      $in: queryFilters.skills.map((s) => s.toLowerCase()),
    };
